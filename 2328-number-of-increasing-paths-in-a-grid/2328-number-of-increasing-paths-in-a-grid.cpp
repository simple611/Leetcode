...
public:
    ...
    long dp(vector<vector<int>>& grid,int i,int j) {
        long result=1;  // Current cell contribution
        if(memo[i][j]!=-1)
            return memo[i][j];
        ...
        for(int d=0;d<4;d++) {
            ...
            if(x>=0&&y>=0&&x<grid.size()&&y<grid[0].size()&&grid[x][y]>grid[i][j])
                result=(result+dp(grid,x,y))%MOD;       // Add total number of cells in longest path
        }
    ...
    int countPaths(vector<vector<int>>& grid) {
        ...
        for(int i=0;i<grid.size();i++)      // For each cell calc the num of increasing paths it can contribute.
